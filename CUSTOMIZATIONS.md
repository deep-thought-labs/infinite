...
- `scripts/setup_module_accounts.sh`: **ModuleAccounts Setup Script** - Standalone script that **executes commands directly** to configure ModuleAccounts in genesis.json for mainnet, testnet, or creative networks according to Cosmos SDK specification. The script reads network-specific module configuration from `*-module-accounts.json` files and automatically creates ModuleAccounts with `name` and initial balances. ModuleAccounts are created with `@type: "/cosmos.auth.v1beta1.ModuleAccount"` containing `base_account`, `name`, and `permissions` fields (always empty array for custom ModuleAccounts, since permissions are only effective when registered in `infinited/config/permissions.go` which requires code changes). The script is idempotent (skips existing ModuleAccounts), supports accounts with 0 tokens, provides detailed error reporting, and validates the genesis file automatically. **Used specifically for network genesis creation process** (not required for regular users running local chains). **Usage**: `./scripts/setup_module_accounts.sh --network <mainnet|testnet|creative> [--genesis-dir <path>]`
- `scripts/genesis-configs/mainnet-module-accounts.json`: **Mainnet ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for mainnet. Array of ModuleAccount objects, each with `name` and `amount_tokens` fields. Custom ModuleAccounts always have empty permissions array (`permissions: []`) since permissions are only effective when registered in `infinited/config/permissions.go` (requires code changes). Used by `setup_module_accounts.sh` when `--network mainnet` is specified.
- `scripts/genesis-configs/testnet-module-accounts.json`: **Testnet ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for testnet. Same structure as mainnet-module-accounts.json but with testnet-specific values. Used by `setup_module_accounts.sh` when `--network testnet` is specified.
- `scripts/genesis-configs/creative-module-accounts.json`: **Creative Network ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for creative network. Same structure as mainnet-module-accounts.json but with creative-specific values. Used by `setup_module_accounts.sh` when `--network creative` is specified.
