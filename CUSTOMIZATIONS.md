...
- `scripts/setup_module_accounts.sh`: **ModuleAccounts Setup Script** - Standalone script that **executes commands directly** to configure pure ModuleAccounts (without vesting) in genesis.json for mainnet, testnet, or creative networks. The script reads network-specific module configuration from `*-vesting.json` files and automatically creates ModuleAccounts with `name`, `permissions`, and initial balances. ModuleAccounts are created as pure accounts (BaseAccount + name + permissions), not vesting accounts, as Cosmos SDK does not natively support ModuleAccounts with vesting. The script is idempotent (skips existing ModuleAccounts), supports accounts with 0 tokens, provides detailed error reporting, and validates the genesis file automatically. **Used specifically for network genesis creation process** (not required for regular users running local chains). **Usage**: `./scripts/setup_module_accounts.sh --network <mainnet|testnet|creative> [--genesis-dir <path>]`
- `scripts/genesis-configs/mainnet-vesting.json`: **Mainnet ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for mainnet. Includes a `pools` array with `name`, `amount_tokens`, `permissions`, and `fee_share_percent` for each ModuleAccount. Also includes `vesting_start_time`, `vesting_end_time`, and `fee_burn_percent` fields reserved for future vesting account script (not used by ModuleAccounts). Used by `setup_module_accounts.sh` when `--network mainnet` is specified.
- `scripts/genesis-configs/testnet-vesting.json`: **Testnet ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for testnet. Same structure as mainnet-vesting.json but with testnet-specific values. Used by `setup_module_accounts.sh` when `--network testnet` is specified.
- `scripts/genesis-configs/creative-vesting.json`: **Creative Network ModuleAccounts Configuration File** - JSON configuration file containing ModuleAccount parameters for creative network. Same structure as mainnet-vesting.json but with creative-specific values. Used by `setup_module_accounts.sh` when `--network creative` is specified.
