...
- `scripts/setup_module_accounts.sh`: **ModuleAccounts Vesting Setup Script** - Standalone script that generates commands (does NOT execute them) for configuring ModuleAccounts with linear vesting in genesis.json for mainnet, testnet, or creative networks. The script reads network-specific vesting configuration from `*-vesting.json` files and generates `infinited genesis add-module-vesting-account` commands with proper atomic unit conversion, UTC timezone formatting, and human-readable duration calculations. **Used specifically for network genesis creation process** (not required for regular users running local chains). **Usage**: `./scripts/setup_module_accounts.sh --network <mainnet|testnet|creative> [--genesis-dir <path>]`
- `scripts/genesis-configs/mainnet-vesting.json`: **Mainnet Vesting Configuration File** - JSON configuration file containing vesting parameters for mainnet ModuleAccounts. Includes `vesting_start_time`, `vesting_end_time`, `fee_burn_percent`, and a `pools` array with `name`, `amount_tokens`, `permissions`, and `fee_share_percent` for each pool. Used by `setup_module_accounts.sh` when `--network mainnet` is specified.
- `scripts/genesis-configs/testnet-vesting.json`: **Testnet Vesting Configuration File** - JSON configuration file containing vesting parameters for testnet ModuleAccounts. Same structure as mainnet-vesting.json but with testnet-specific values. Used by `setup_module_accounts.sh` when `--network testnet` is specified.
- `scripts/genesis-configs/creative-vesting.json`: **Creative Network Vesting Configuration File** - JSON configuration file containing vesting parameters for creative network ModuleAccounts. Same structure as mainnet-vesting.json but with creative-specific values. Used by `setup_module_accounts.sh` when `--network creative` is specified.
