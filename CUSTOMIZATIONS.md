...
- `scripts/setup_module_accounts.sh`: **ModuleAccounts Setup Script** - Standalone script that generates commands (does NOT execute them) for configuring pure ModuleAccounts (without vesting) in genesis.json for mainnet, testnet, or creative networks. The script reads network-specific module configuration from `*-vesting.json` files and generates commands to create ModuleAccounts with `name`, `permissions`, and initial balances. ModuleAccounts are created as pure accounts (BaseAccount + name + permissions), not vesting accounts, as Cosmos SDK does not natively support ModuleAccounts with vesting. **Used specifically for network genesis creation process** (not required for regular users running local chains). **Usage**: `./scripts/setup_module_accounts.sh --network <mainnet|testnet|creative> [--genesis-dir <path>]`
