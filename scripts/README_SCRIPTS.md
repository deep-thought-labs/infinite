...
**`setup_genesis.sh`** - Configure an existing genesis.json file for Mainnet or Testnet.

See [config/genesis/README.md](../config/genesis/README.md) for detailed documentation and configuration options.

**Usage:**
```bash
./scripts/setup_genesis.sh [mainnet|testnet] <genesis-file-path>
```

**Prerequisites:**
1. The node must be initialized first:
   ```bash
   infinited init <moniker> --chain-id <chain-id> --home <home-dir>
   ```
2. This script modifies the existing genesis.json file generated by the init command.

**Examples:**
```bash
# Configure Mainnet genesis (after initializing the node)
...
# Configure Testnet genesis (after initializing the node)
infinited init test-node --chain-id infinite_421018001-1 --home ~/.infinited_testnet
./scripts/setup_genesis.sh testnet ~/.infinited_testnet/config/genesis.json

# Custom home directory
infinited init production-node --chain-id infinite_421018-1 --home /custom/path/.infinited
...
**What it does:**
- Reads configuration from `config/genesis/<network>.yaml`
...
- Validates the modified genesis file

**Note:** This script does NOT initialize the node. You must run `infinited init` first to create the initial genesis.json file.
