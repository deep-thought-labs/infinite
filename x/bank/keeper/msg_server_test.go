...
func TestMsgServer_SetDenomMetadata_Success(t *testing.T) {
	...
	require.Equal(t, "denom", attrs[0].Key)
	require.Equal(t, md.Base, attrs[0].Value)
	require.Equal(t, "name", attrs[1].Key)
	require.Equal(t, md.Name, attrs[1].Value)
	require.Equal(t, "symbol", attrs[2].Key)
	require.Equal(t, md.Symbol, attrs[2].Value)
}
