...
**For step-by-step instructions**: See [GENESIS.md](GENESIS.md) instead.

---

## Table of Contents

1. [Where are Genesis Parameters Configured?](#where-are-genesis-parameters-configured)
...
10. [Key Differences: Development vs Production](#key-differences-development-vs-production)
...
- **Base Denom**: The smallest unit (equivalent to "wei" in Ethereum)
  - Example: `drop` (mainnet), `tdrop` (testnet), `cdrop` (creative)
  - Used for all internal operations

- **Display Denom**: The unit shown to users
  - Example: `Improbability` (mainnet), `TestImprobability` (testnet), `CreativeImprobability` (creative)
    ...
    "gov": {
      "params": {
        "min_deposit": [
          {
            "denom": "drop",
            "amount": "1000000000000000000"      // 1 token (adjust as needed)
          }
        ],
        ...
        "expedited_min_deposit": [
          {
            "denom": "drop",
            "amount": "5000000000000000000"      // 5 tokens for expedited proposals
          }
        ],
