...
**⚠️ IMPORTANT**: 
- This guide is **specifically for mainnet/testnet creation** (one-time setup process)
...
- The examples in the guides show how to populate the genesis with dummy accounts or dummy transactions for development. This document focuses on what's needed for a **real production Mainnet**
...
2. **Apply Infinite Drive Customizations** (for mainnet/testnet setup):
   ```bash
   ...
   **⚠️ CONTEXT**: This step is part of the **mainnet/testnet creation process**. The genesis generated by `infinited init` uses default Cosmos SDK values (like "stake" denomination). For mainnet/testnet, you need to run `customize_genesis.sh` to apply Infinite Drive customizations.
   
   **Note for regular users**: If you're just running a local development chain, use `local_node.sh` instead, which handles all genesis customization automatically.
...
### Step 2: Apply Infinite Drive Customizations (Mainnet/Testnet Setup)

**Context**: This step is part of the **mainnet/testnet creation process**. After `infinited init`, the generated genesis uses default Cosmos SDK values (e.g., "stake" denomination). For mainnet/testnet, you need to apply Infinite Drive customizations:
...
**Note**: Regular users running local development chains should use `local_node.sh` instead, which handles genesis customization automatically.
...
- **Genesis Customization Script**: `scripts/customize_genesis.sh` - **For mainnet/testnet creation**: Execute this after `infinited init` during the mainnet/testnet setup process
