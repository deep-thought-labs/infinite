...
After running `infinited init`, you must **manually modify** the `genesis.json` file for Mainnet:

1. **Denominations (Denoms)**:
   ```bash
   # Example using jq (as in local_node.sh):
...
2. **Customize for Mainnet**:
   - Use `jq` to modify denoms
   - Use `sed` or manual editing to change governance periods
   - Use CLI commands to add accounts and validators
   - Manually edit specific parameters
...
**Practical example**: See `local_node.sh` lines 233-256 to see how Genesis is customized after `infinited init`.
...
Use `jq` or manual editing to configure all parameters mentioned above.
