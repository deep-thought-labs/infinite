...
En CI, el workflow **Tests** ejecuta la cobertura en **cuatro bloques** (checks tipo **`test-unit-cover (infinited-integration)`**, targets `make test-unit-cover-*`): ver [TESTING.md — Granular coverage blocks](../../guides/development/TESTING.md#granular-coverage-blocks-test-unit-cover) y [CI_CD.md — Path filtering](../../guides/infrastructure/CI_CD.md#path-filtering-docs-only-changes). Un fallo por timeout en **`infinited/tests/integration`** aparece entonces en el job **`test-unit-cover (infinited-integration)`**, sin enmascarar el resto de bloques.
