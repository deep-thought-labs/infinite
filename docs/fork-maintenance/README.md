...
| [**MERGE_STRATEGIES.md**](MERGE_STRATEGIES.md) | Alternativas de integración, zonas protegidas, **§4 GitHub Actions** (alinear workflows con `upstream/main`, conservar `release.yml` + **`iid-v*`** + `build.yml`/`Makefile` de versión, deltas `infinited`), **§4.6** [`.markdownlint.yml`](../../.markdownlint.yml) + [Makefile](../../Makefile) (`markdownlint_cli2_version`, `make lint-md` / `make lint`) y documentación por ciclo. |
...
| [**logs/**](logs/) | Bitácoras de merge upstream (**crear al inicio**, **completar al cierre** — [logs/README.md](logs/README.md)) y, si aplica, **features del fork** (p. ej. [Hyperlane](logs/2026-04-03-hyperlane-integration.md), [Infinite Bank](logs/2026-04-04-infinite-bank.md)). |
