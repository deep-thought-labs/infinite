...
El `Makefile` define **`SYSTEMTEST_LEGACY_TAG`**: release de GitHub desde el que se obtiene el binario “viejo” para el escenario de upgrade on-chain (descarga verificada; no hay `git checkout` ni compilación local de ese tag). **En el fork Infinite Drive** el valor por defecto apunta al **release publicado en este repositorio** (p. ej. **`v0.1.10`**), no al tag que documente cosmos/evm (`v0.5.1`, etc.), que puede **no existir** en el fork. Debe existir el **release** (artefactos + `checksums.txt`) en GitHub para ese tag. Detalle y tabla de verificación: bitácora [2026-03-merge-upstream-main.md](logs/2026-03-merge-upstream-main.md#system-tests-y-upgrades-on-chain-fork).
